...
const TeacherSchema = z.object({
  ...
  login_email: z.string().trim().optional().or(z.literal("")),
  login_password: z.string().trim().optional().or(z.literal("")),
...
async function saveAcademicAssignments(args: {
  teacherId: string;
  termId: number | null;
  classId: number | null;
  ...
  if (assignAsSubjectTeacher && subjectId) {
    const existingSubjectAssignment = await sb()
    ...
    if (existingSubjectAssignment.error) {
      redirect(`${back}?err=${enc(existingSubjectAssignment.error.message)}`);
    }

    if (existingSubjectAssignment.data?.id) {
      const { error } = await sb()
        .from("teacher_assignments")
        .update({ teacher_id: teacherId })
        .eq("id", existingSubjectAssignment.data.id);
...
export async function upsertTeacherProfile(formData: FormData) {
  ...
  const parsed = TeacherSchema.safeParse({
    ...
    salary_amount: formData.get("salary_amount") ? Number(formData.get("salary_amount")) : null,
    ...
    create_portal_user: formData.get("create_portal_user") ? true : false,
    login_email: String(formData.get("login_email") ?? "").trim(),
    login_password: String(formData.get("login_password") ?? "").trim(),
  ...
  if (!data.id && data.create_portal_user) {
    if (!data.login_email || !data.login_password || data.login_password.length < 8) {
      redirect(`${back}?err=${enc("Login email and password are required when creating portal login now.")}`);
    }
  }
  ...
  if (data.create_portal_user) {
    ...
    if (created.error || !created.data.user) {
      redirect(`/portal/admin/staff-internal/new?err=${enc(created.error?.message ?? "Failed to create portal login.")}`);
    }
    ...
    if (profileError) {
      ...
      redirect(`/portal/admin/staff-internal/new?err=${enc(profileError.message)}`);
    }
  }

  const newTeacherId = crypto.randomUUID();

  const { error: teacherInsertError } = await sb().from("teachers").insert({
    id: newTeacherId,
    user_id: newUserId,
    ...payload,
  ...
  if (teacherInsertError) {
    ...
    redirect(`/portal/admin/staff-internal/new?err=${enc(teacherInsertError.message)}`);
  }
  ...
  await saveAcademicAssignments({
    ...
    assignAsClassTeacher: data.assign_as_class_teacher ?? false,
    assignAsSubjectTeacher: data.assign_as_subject_teacher ?? false,
    back: `/portal/admin/staff-internal/${newTeacherId}`,
  });

  revalidatePath("/portal/admin/staff-internal");
  revalidatePath("/portal/admin/academics");
  revalidatePath("/portal/admin/users");
  redirect(`/portal/admin/staff-internal/${newTeacherId}?ok=${enc("Staff profile created.")}`);
}
...
export async function setTeacherActive(formData: FormData) {
  ...
  if (teacherRes.data?.user_id) {
    await sb().from("profiles").update({ is_active: isActive }).eq("id", teacherRes.data.user_id);
  }
...
export async function createTeacherPortalLogin(formData: FormData) {
  ...
  if (teacherRes.error) redirect(`${back}?err=${enc(teacherRes.error.message)}`);
  if (!teacherRes.data) redirect(`${back}?err=${enc("Teacher not found.")}`);
  if (teacherRes.data.user_id) redirect(`${back}?err=${enc("This teacher already has a portal login.")}`);
...
export async function resetTeacherPortalPassword(formData: FormData) {
  ...
  if (teacherRes.error) redirect(`${back}?err=${enc(teacherRes.error.message)}`);
  if (!teacherRes.data) redirect(`${back}?err=${enc("Teacher not found.")}`);
  if (!teacherRes.data.user_id) redirect(`${back}?err=${enc("This teacher does not have a portal login yet.")}`);

  const reset = await sb().auth.admin.updateUserById(teacherRes.data.user_id, {
    password: parsed.data.password,
  });

  if (reset.error) redirect(`${back}?err=${enc(reset.error.message)}`);
...
export async function deleteTeacherProfile(formData: FormData) {
  ...
  const id = String(formData.get("id") ?? "").trim();
  if (!id) redirect(`/portal/admin/staff-internal?err=${enc("Missing teacher id.")}`);
  ...
  if (teacherRes.error) redirect(`/portal/admin/staff-internal?err=${enc(teacherRes.error.message)}`);
  if (!teacherRes.data) redirect(`/portal/admin/staff-internal?err=${enc("Teacher not found.")}`);

  const userId = teacherRes.data.user_id ?? null;

  // With the cascade SQL in place, this delete becomes much simpler.
  const { error: teacherDeleteError } = await sb().from("teachers").delete().eq("id", id);
  if (teacherDeleteError) redirect(`/portal/admin/staff-internal?err=${enc(teacherDeleteError.message)}`);

  if (userId) {
    const { error: profileDeleteError } = await sb().from("profiles").delete().eq("id", userId);
    if (profileDeleteError) redirect(`/portal/admin/staff-internal?err=${enc(profileDeleteError.message)}`);

    const authDelete = await sb().auth.admin.deleteUser(userId);
    if (authDelete.error) redirect(`/portal/admin/staff-internal?err=${enc(authDelete.error.message)}`);
  }
