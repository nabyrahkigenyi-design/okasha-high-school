...
const Schema = z.object({
  ...
  folder: z.enum(["news", "staff", "docs", "assignments", "students"]).default("news"),
});
...
export async function POST(req: Request) {
  try {
    ...
    // teachers can only upload assignment files
    if (role === "teacher" && parsed.data.folder !== "assignments") {
      return NextResponse.json({ ok: false, error: "Forbidden" }, { status: 403 });
    }

    // admins can upload to all allowed folders, including students
